...
# cluster 17 screen vgg16_bn AdamW 1
# NAME="bottomup_swig_div3_vgg16bn"
# OPTIM="AdamW"
# rm -f log/${NAME}.log dist/${NAME}
# rm -rf log/${NAME}
# CUDA_VISIBLE_DEVICES="2,3" python -m torch.distributed.launch --nproc_per_node=2 --use_env \
#     main.py --batch_size 8 --image_resize 512 --backbone vgg16_bn \
#     --epochs 50 --optimizer ${OPTIM} --dataset_file swig --image_dir images_512 --lr 1e-4 --lr_backbone 1e-5 \
#     --dist_url file://${PWD}/dist/${NAME} \
#     --output_dir log/${NAME} |\
#     while IFS= read -r line; do printf '%s %s\n' "$(date +%m:%d-%T)" "$line"; done |\
...
#     main.py --batch_size 32 --image_resize 256 --backbone resnet50 \
#     --epochs 50 --optimizer ${OPTIM} --dataset_file swig --image_dir images_512 --lr 1e-4 --lr_backbone 1e-5 \
#     --dist_url file://${PWD}/dist/${NAME} \
#     --output_dir log/${NAME} |\
#     while IFS= read -r line; do printf '%s %s\n' "$(date +%m:%d-%T)" "$line"; done |\
...
NAME="bottomup_swig_div3_resized256_vgg16bn"
OPTIM="AdamW"
rm -f log/${NAME}.log dist/${NAME}
rm -rf log/${NAME}
CUDA_VISIBLE_DEVICES="0,1,2,3" python -m torch.distributed.launch --nproc_per_node=4 --use_env \
    main.py --batch_size 2 --image_resize 256 --backbone vgg16_bn \
    --epochs 50 --optimizer ${OPTIM} --dataset_file swig --image_dir images_512 --lr 1e-4 --lr_backbone 1e-5 \
    --dist_url file://${PWD}/dist/${NAME} \
    --output_dir log/${NAME} |\
    while IFS= read -r line; do printf '%s %s\n' "$(date +%m:%d-%T)" "$line"; done |\
